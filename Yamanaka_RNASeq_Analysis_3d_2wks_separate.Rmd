...
p <- plotPCA2(vsd_blind_3d,
         intgroup = "genotype",
         ntop=500,
         PCs=c(x=1,y=2),
         colour = group)  +
  scale_color_manual(values = c("Ctrl" = "#C77CFF", "CrePos" = "#7CAE00"))
...
p <- plotPCA2(vsd_blind_2wks,
              intgroup = "genotype",
              ntop=500,
              PCs=c(x=1,y=2),
              colour = group) +
  scale_color_manual(values = c("Ctrl" = "#00BFC4", "CrePos" = "#F8766D"))

ggsave("results/final_figures/PCA_2wks.pdf", plot = p, width = 6, height = 4)
p
...
write_xlsx(res_3d_df, "results/differential_expression/res_3d_all_genes.xlsx")
```
...
write_xlsx(res_2wks_df, "results/differential_expression/res_2wks_all_genes.xlsx")
```
...
-- edit: I will use a log2FC threshold of 0.5 for the paper.

```{r strong GO effects for 3 day analysis}
#upregulated genes
de_genes_up_strong <- res_3d_df |>
  dplyr::filter(!is.na(padj), padj <= 0.05, log2FoldChange > 0.5) |>
  dplyr::pull(gene_symbol) |>
  unique()

#downregulated genes
de_genes_dn_strong <- res_3d_df |>
  dplyr::filter(!is.na(padj), padj <= 0.05, log2FoldChange < -0.5) |>
  dplyr::pull(gene_symbol) |>
  unique()
...
#EDIT: using now the strong effect - meaning a logFC of 0.5 as threshold

# Plot 1 - Upregulated
p1 <- dotplot(ego_result_up_strong, showCategory=10) +
  ggtitle("GO Analysis - Upregulated Genes")
p1
ggsave("results/final_figures/3d_GO_upregulated.pdf", plot = p1, width = 8, height = 6)

# Plot 2 - Downregulated
p2 <- dotplot(ego_result_dn_strong, showCategory=10) +
  ggtitle("GO Analysis - Downregulated Genes")
p2
ggsave("results/final_figures/3d_GO_downregulated.pdf", plot = p2, width = 8, height = 6)
...
ggsave("results/final_figures/2wks_GO_upregulated_logFC1.pdf", plot = p1, width = 8, height = 6)

# Plot 2 - Downregulated
p2 <- dotplot(ego_result_dn_strong, showCategory=10) +
  ggtitle("GO Analysis - Downregulated Genes")

ggsave("results/final_figures/2wks_GO_downregulated_logFC1.pdf", plot = p2, width = 8, height = 6)
...
write.csv(sig_bp,file = "results/differential_expression/GSEA/GSEA_3d_sig_bp.csv",
          row.names = FALSE)
...
write.csv(sig_bp,file = "results/differential_expression/GSEA/GSEA_2wks_sig_bp.csv",
          row.names = FALSE)
...
#Change Pou5f1 to Oct4 for Volcano Plot
res_3d_df$gene_symbol[res_3d_df$gene_symbol == "Pou5f1"] <- "Oct4"
...
p1 <- ggplot(res_plot, aes(x = log2FoldChange, y = -log10(padj), color = significant)) +
  ...
  # Top 20 Genes - show.legend = FALSE hinzufügen!
  geom_text_repel(data = top_genes, aes(label = gene_symbol), 
                  size = 3, show.legend = FALSE) +
  # Sox2 - show.legend = FALSE hinzufügen!
  geom_text_repel(data = sox2_gene, aes(label = gene_symbol), 
                  size = 3,  show.legend = FALSE) +
  theme_minimal() +
  labs(x = "log2 Fold Change", y = "-log10(p-value)", color = "Significance") +
  ggtitle("Volcano Plot - 3 days after induction")

ggsave("results/final_figures/3d_volcano_logFC1.pdf", plot = p1, width = 6, height = 5)
...
p2 <- ggplot(res_plot, aes(x = log2FoldChange, y = -log10(padj), color = significant)) +
  geom_point(alpha = 0.6, size = 1.5) +
  scale_color_manual(values = c("Up" = "red", "Down" = "blue", "NotSig" = "grey")) +
  geom_vline(xintercept = c(-lfc_thresh, lfc_thresh), linetype = "dashed") +
  geom_hline(yintercept = -log10(p_thresh), linetype = "dashed") +
  ...
  theme_minimal() +
  labs(x = "log2 Fold Change", y = "-log10(p-value)", color = "Significance") +
  ggtitle("Volcano Plot - 2 weeks after induction")

ggsave("results/final_figures/2wks_volcano_logFC1.pdf", plot = p2, width = 6, height = 5)
...
And performing KEGG analysis

```{r KEGG analysis 3 day}

#upregulated genes
de_genes_up_strong_KEGG_3d <- res_3d_df |>
  dplyr::filter(!is.na(padj), padj <= 0.05, log2FoldChange > 1) |>
  dplyr::pull(entrez_id) |>
  unique()

#downregulated genes
de_genes_dn_strong_KEGG_3d <- res_3d_df |>
  dplyr::filter(!is.na(padj), padj <= 0.05, log2FoldChange > -1) |>
  dplyr::pull(entrez_id) |>
  unique()

#set "Universe" - defined as all genes that are actually expressed
uni_genes_KEGG_3d <- res_3d_df |>
  dplyr::filter(!is.na(padj)) |>
  dplyr::pull(entrez_id) |>
  unique()

# KEGG Analysis 3d up
kegg_res_up_3d <- enrichKEGG(
  gene          = de_genes_up_strong_KEGG_3d,
  organism      = "mmu",
  keyType       = "ncbi-geneid",
  universe      = uni_genes_KEGG_3d,
  pAdjustMethod = "BH",
  pvalueCutoff  = 0.05,
  qvalueCutoff  = 0.2
)

# KEGG Analysis 3d down
kegg_res_dn_3d <- enrichKEGG(
  gene          = de_genes_dn_strong_KEGG_3d,
  organism      = "mmu",
  keyType       = "ncbi-geneid",
  universe      = uni_genes_KEGG_3d,
  pAdjustMethod = "BH",
...
kegg_up_3d <- dotplot(kegg_res_up_3d, showCategory=10,  font.size = 14, title="KEGG Analysis - Upregulated genes")
kegg_up_3d
ggsave("results/final_figures/3d_KEGG_up_logFC1.pdf", plot = kegg_up_3d, width = 8, height = 6)

kegg_dn_3d <- dotplot(kegg_res_dn_3d, showCategory=10,  font.size = 14, title="KEGG Analysis - Downregulated genes")
kegg_dn_3d
ggsave("results/final_figures/3d_KEGG_dn_logFC1.pdf", plot = kegg_dn_3d, width = 8, height = 6)


```


```{r KEGG Analysis 2 weeks}


#upregulated genes
de_genes_up_strong_KEGG_2wks <- res_2wks_df |>
  dplyr::filter(!is.na(padj), padj <= 0.05, log2FoldChange > 1) |>
  dplyr::pull(entrez_id) |>
  unique()

#downregulated genes
de_genes_dn_strong_KEGG_2wks <- res_2wks_df |>
  dplyr::filter(!is.na(padj), padj <= 0.05, log2FoldChange > -1) |>
  dplyr::pull(entrez_id) |>
  unique()

#set "Universe" - defined as all genes that are actually expressed
uni_genes_KEGG_2wks <- res_2wks_df |>
  dplyr::filter(!is.na(padj)) |>
  dplyr::pull(entrez_id) |>
  unique()

# KEGG Analysis 2wks up
kegg_res_up_2wks <- enrichKEGG(
  gene          = de_genes_up_strong_KEGG_2wks,
  organism      = "mmu",
  keyType       = "ncbi-geneid",
  universe      = uni_genes_KEGG_2wks,
  pAdjustMethod = "BH",
  pvalueCutoff  = 0.05,
  qvalueCutoff  = 0.2
)

# KEGG Analysis 2wks down
kegg_res_dn_2wks <- enrichKEGG(
  gene          = de_genes_dn_strong_KEGG_2wks,
  organism      = "mmu",
  keyType       = "ncbi-geneid",
  universe      = uni_genes_KEGG_2wks,
  pAdjustMethod = "BH",
...
kegg_up_2wks <- dotplot(kegg_res_up_2wks, showCategory=10,  font.size = 14, title="KEGG Analysis - Upregulated genes")
kegg_up_2wks
ggsave("results/final_figures/2wks_KEGG_up_logFC1.pdf", plot = kegg_up_2wks, width = 8, height = 6)

kegg_dn_2wks <- dotplot(kegg_res_dn_2wks, showCategory=10,  font.size = 14, title="KEGG Analysis - Downregulated genes")
kegg_dn_2wks
ggsave("results/final_figures/2wks_KEGG_dn_logFC1.pdf", plot = kegg_dn_2wks, width = 8, height = 6)
...
## Further analysis - fatty acid metabolism


```{r extract genes belonging to the Top 30 clusters}

top30_up_2wks <- as.data.frame(ego_result_up_strong)[1:30, ]
genes_list <- strsplit(top30_up_2wks$geneID, "/")
names(genes_list) <- top30_up_2wks$Description
unique_genes <- unique(unlist(genes_list)) #only keep unique genes

#put genes into a table
df_top30_up_2wks <- top30_up_2wks %>%
  select(Description, geneID) %>%
  separate_rows(geneID, sep="/")

res_2wks_df_GOTopGenes <- res_2wks_df[res_2wks_df$gene_symbol %in% unique_genes,]

```


```{r Fatty acid metabolism}
#taking gene list from Susztaks paper (Kang et al. 2015)
fatty_acid_genes <- c("Acat1", "Aldh3a2", "Acaa2", "Acaa1b", "Acaa1a", "Cyp4a10", "Aldh7a1", "Echs1", "Fads1",
                      "Aldh1l1", "Adhf", "Aldh2", "Adh5", "Acox1", "Hadh", "Aldh9a1", "Acadl",
                      "Gcdh", "Cpt1a", "Cpt2", "Acsl1", "Acat2", "Ehhadh", "Hadha", "Acadsr",
                      "Acsl5", "Acsl4", "Adh1")

res_2wks_df_fatty_acid_sig <- res_2wks_df[res_2wks_df$gene_symbol %in% fatty_acid_genes & 
  res_2wks_df$padj < 0.05,]
res_2wks_df_fatty_acid <- res_2wks_df[res_2wks_df$gene_symbol %in% fatty_acid_genes,]

res_3d_df_fatty_acid <- res_3d_df[res_3d_df$gene_symbol %in% fatty_acid_genes,]


```

```{r Make a heatmap for these genes}

vsd_true_2wks <- vst(dds_2wks, blind=FALSE) # this time use the FALSE argument to keep differences
mat_2wks <- assay(vsd_true_2wks)

#map EntrezIDs to gene symbols
entrez_ids_2wks <- rownames(mat_2wks)
gene_symbols_2wks <- mapIds(org.Mm.eg.db,
                       keys = entrez_ids_2wks,
                       column = "SYMBOL",
                       keytype = "ENTREZID",
                       multiVals = "first")  # take first if multiple matches
...
#Add gene symbols to the results table
rownames(mat_2wks) <- gene_symbols_2wks

# remove NA
mat_2wks <- mat_2wks[!is.na(rownames(mat_2wks)), ]

genes_of_interest <- res_2wks_df_fatty_acid$gene_symbol
genes_of_interest <- res_2wks_df_fatty_acid_sig$gene_symbol


mat_subset <- mat_2wks[rownames(mat_2wks) %in% genes_of_interest, ]

#scale the matrix subset for better visualization (optional)
mat_scaled <- t(scale(t(mat_subset)))

gene_order <- c(
  # FA import
  "Cpt1a", "Cpt2", "Acsl1", "Acsl4", "Acsl5",
  
  # mitochondrial FAO
  "Acadl", "Acaa2", "Hadha", "Hadh", "Echs1",
  
  # peroxisomal FAO
  "Acox1", "Acaa1a", "Acaa1b", "Ehhadh",
  ...
  # detox
  "Aldh1l1", "Aldh2", "Aldh3a2", "Aldh7a1", "Aldh9a1",
...
# 3. Samples nach Genotyp sortieren
# ------------------------------
genotype_factor <- factor(colData(dds_2wks)$genotype,
                          levels = c("Ctrl", "CrePos"))

# explizite Reihenfolge: Ctrl zuerst, dann CrePos
sample_order <- c(
  which(genotype_factor == "Ctrl"),
  which(genotype_factor == "CrePos")
)

mat_scaled <- mat_scaled[, sample_order]

# ------------------------------
# 4. Column-Annotation vorbereiten
# ------------------------------
annotation_col <- data.frame(
  Genotype = genotype_factor[sample_order]
)
rownames(annotation_col) <- colnames(mat_scaled)

# ------------------------------
# 5. Row-Annotation (optional: Pathway/Kategorie)
# ------------------------------
...
pheatmap(
  mat_scaled,
  cluster_rows = TRUE,      # Gene bleiben in biologischer Reihenfolge
  cluster_cols = FALSE,      # Columns bleiben in Genotyp Reihenfolge
