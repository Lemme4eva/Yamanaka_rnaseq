# ==============================================================================
# Extraktion der Top-Gene für Collecting duct (CCD, OMCD, IMCD)
...
cd_pattern <- c("CCD", "OMCD", "IMCD")
cd_columns <- colnames(expr_data)[grepl(paste(cd_pattern, collapse = "|"),
                                        colnames(expr_data))]

cat("Collecting Duct Spalten:\n")
print(cd_columns)
cat("Anzahl CD-Spalten:", length(cd_columns), "\n\n")

# Alle anderen Segmente (zum Vergleich)
other_segments <- c("PTS1", "PTS2", "PTS3", "DTL", "ATL", "MTAL", "CTAL", "DCT", "CNT")
...
expr_data$CD_mean <- rowMeans(expr_data[, cd_columns], na.rm = TRUE)
...
expr_data$CD_vs_Other_FC <- log2((expr_data$CD_mean + 1) / (expr_data$Other_mean + 1))

# 4. TOP-GENE IDENTIFIZIEREN
# ==============================================================================

# Sortieren nach PT-Expression
expr_sorted <- expr_data[order(-expr_data$CD_mean), ]

# Top 100 Gene basierend auf PT-Expression
top_100_CD <- head(expr_sorted, 100)

# Top 50 Gene
top_50_CD <- head(expr_sorted, 50)

# Optionale Filterung: Gene, die spezifisch für PT sind
# (höhere Expression in PT als in anderen Segmenten)
cd_specific <- expr_data[expr_data$CD_vs_Other_FC > 0, ]  # FC > 0 bedeutet PT > Other
cd_specific <- cd_specific[cd_specific$CD_mean > 200, ] #keep genes with minimal expression counts of 200 and more
cd_specific_sorted <- cd_specific[order(-cd_specific$CD_vs_Other_FC), ]

top_50_specific <- head(cd_specific_sorted, 50)
...
top50_specific_summary <- data.frame(
  ...
  CD_mean_expression = top_50_specific$CD_mean,
  Other_segments_mean = top_50_specific$Other_mean,
  CD_vs_Other_logFC = top_50_specific$CD_vs_Other_FC,
  row.names = NULL
)
write.csv(top50_specific_summary, "results/cell-specific analyses/Top50_CD_specific_genes_summary.csv", row.names = FALSE)
