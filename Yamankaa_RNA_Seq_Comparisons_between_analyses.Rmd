...
```{r preparing up- and downregulated gene subsets}
...
Interestingly, there are more differentially expressed genes long-term. However, most of these only have a small fold change. If we change the log fold 2 change to 0.5, then the numbers are very different.
...
venn
ggsave("results/final_figures/venn_upregulated_genes_logFC1.pdf", plot = venn, width = 6, height = 4, units = "in", dpi = 300)
```
...
venn
ggsave("results/final_figures/venn_downregulated_genes_logFC1.pdf", plot = venn, width = 6, height = 4, units = "in", dpi = 300)
```
...
combined <- venn_up_down + venn_down_up +
  plot_annotation(
    ...
    subtitle = "CrePos vs Ctrl (padj < 0.05, |log2FC| > 0.5)",
    theme = theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold"),
                  plot.subtitle = element_text(hjust = 0.5, size = 10))
  )
...
genes_up_consist <- intersect(de_genes_up_strong_3d, 
                                 de_genes_up_strong_2wks)

# Gene DOWN at both timepoints
genes_dn_consist <- intersect(de_genes_dn_strong_3d, 
                                 de_genes_dn_strong_2wks) 
...
DT::datatable(head(ego_result_up_consist)) 
...
p1 <- dotplot(ego_result_up_consist, showCategory=10) +
  ggtitle("GO Analysis - consistently upregulated genes")
p1
ggsave("results/final_figures/consist_GO_upregulated.pdf", plot = p1, width = 8, height = 6)

p2 <- dotplot(ego_result_dn_consist, showCategory=10) +
  ggtitle("GO Analysis - consistently downregulated genes")
p2
ggsave("results/final_figures/consist_GO_downregulated.pdf", plot = p2, width = 8, height = 6)

dotplot(ego_result_up_consist, showCategory=10) +
  ggtitle("GO - Consistently upregulated genes")

dotplot(ego_result_dn_consist, showCategory=10) +
  ggtitle("GO - Consistently downregulated genes")
